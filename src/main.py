...
class DataExtractor_MSS:
    def __init__(self, server: str = 'BI-DEPT01', database: str = 'master', trusted_connection: str = 'yes'):
        self.connection_string_pyodbc = f'DRIVER=ODBC Driver 17 for SQL Server;SERVER={server};DATABASE={database};TRUSTED_CONNECTION={trusted_connection};'
        self.connection_string_sqlalchemy = f'mssql+pyodbc:///?odbc_connect={self.connection_string_pyodbc}'
        self.engine = create_engine(self.connection_string_sqlalchemy)
...
def csv_task(sql_query_path: str, csv_path: str, server: str, database: str):
    try:
        if not sql_query_path or not csv_path:
            return "Не выбран sql скрипт или путь для csv файла"
        # data_extractor = DataExtractor_Postgres()
        data_extractor = DataExtractor_MSS(server, database)
        sql_query = text(open(sql_query_path).read())
        data = data_extractor.extract_data(sql_query)
...
def dependency(dependency_path: str, server: str, database: str):
    try:
        ...
        data_extractor = DataExtractor_MSS(server, database)
        sql_query = text(open(dependency_path).read())
        df = data_extractor.extract_data(sql_query)
        # print(df)
        if df[df['DWH'] != 1].empty:
            # Зависимости выполнены
            # print("Зависимости выполнены")
            return True
        else:
...
def main(page: ft.Page):
    ...
    def execute_task(task):
        ...
        if not dependencies_met:
            # Если зависимости не выполнены, переносим задачу в очередь через 900 секунд
            thread = threading.Timer(900, lambda: task_queue.put(task))
            thread.start()
            task.thread = thread
    ...
    def active_(task):
        ...
        # Планируем событие с учетом ожидания количества сек: seconds_to_wait
    ...
    class pipeline_row(ft.DataRow):
        def __init__(self,
            ...
            self.segment_but = segment_but()  # Кнопка для выбора #once или #daily обновления задачи(pipeline-а)
